...
test("no unreachable connector panel is left in the Settings folder", () => {
  assert.equal(
    existsSync(join(root, "src/settings/McpSection.tsx")),
    false,
  ...
  const hits = readdirSync(join(root, "src/settings"))
    .filter((f) => /\.tsx?$/.test(f))
    .filter((f) => read(`src/settings/${f}`).includes("McpSection"));
...
test("the checklist still calls a connector page in Settings a failure", () => {
  ...
  const checklist = read("qa/UA-FEATURE-CHECKLIST.md");
...
test("disconnecting a cloud provider asks before wiping the key", () => {
  const src = read("src/settings/AiProvidersSection.tsx");
