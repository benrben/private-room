...
(`Path(__file__).resolve().parents[3]` from this file IS the repo root
`.../private-room` — verified on this exact checkout: `parents[0]` = `rec/`,
`[1]` = `arcelle_sidecar/`, `[2]` = `sidecar/`, `[3]` = the repo root
containing `src-tauri/`), NOT a sidecar-local `resources/models` directory
