...
``electron/main/mcpBridge.ts`` (a fresh TypeScript port of
...
electron/main/mcpBridgeRunner.ts`` — this workspace ships ``vite-node`` via
...
``electron-migration/electron-app/electron/main/mcpBridge.test.ts``.
"""
...
# sidecar/tests/test_mcp_bridge_wire_compat.py -> the repo root is two levels
# up from `tests/`.
_REPO_ROOT = Path(__file__).resolve().parents[2]
_ELECTRON_APP_DIR = _REPO_ROOT / "electron-migration" / "electron-app"
_RUNNER_RELATIVE = "electron/main/mcpBridgeRunner.ts"

#: How long to wait for `vite-node` to cold-start and print its port line.
_STARTUP_TIMEOUT = 60.0

#: Must match MAX_REQUEST_BODY in electron/main/mcpBridge.ts (and
...
async def test_a_bearer_that_merely_starts_with_the_real_token_is_rejected() -> None:
    ...
    itself in electron/main/mcpBridge.test.ts. What this proves is the part a
