...
// Byte-faithful port of src/settings/useModelManagement.ts: the ONLY change
// is the import source below. `listen` used to come from
// @tauri-apps/api/event; it is now the same-named, same-signature adapter
// api.electron.ts exports (merged into the existing "../api" import rather
// than added as a second import of the same module). No runtime code in this
// file changes. See the KNOWN GAP comment at the one call site below.
import { useEffect, useRef, useState } from "react";
import type React from "react";
import { api, ModelCaps, SttStatus, recommendedModels, ensureEmbedModel, listen } from "../api";
...
export function useModelManagement(
  ...
  useEffect(() => {
    ...
    // adapter's `async` note), and the cleanup's `unlisten.then(...)` below
    // surfaces that as an unhandled rejection rather than taking the effect
    // down mid-body.
