// ELECTRON MIGRATION DRAFT — merged. NOT WIRED IN / NOT AUTHORIZED.
// Byte-faithful port of src/workspace/effects.ts: the ONLY change is the
// `listen` import source below (was @tauri-apps/api/event, now the
// same-named, same-signature adapter api.electron.ts exports, merged into
// the existing "../api" import). No runtime code in this file changes. See
// the KNOWN GAP comment at the one direct call site below.
//
// TODO(window/webview-bridge): `getCurrentWindow` (@tauri-apps/api/window)
// and `getCurrentWebview` (@tauri-apps/api/webview) are LEFT AS-IS below,
// unmigrated, on purpose — same reasoning as chooseOpenPath/chooseSavePath
// in api.electron.ts. Neither preload candidate exposes anything beyond
// invoke/on (both say so explicitly in their own module docs: no
// dialog/shell/app/window/webview surface yet, that is separate follow-up
// design work, not this batch's, and not this task's four named items or its
// "@tauri-apps/api/event and @tauri-apps/api/core" grep scope). Inventing a
// window-title-setter or a drag/drop bridge here would be exactly the kind
// of guess this codebase's own convention refuses.
import { useEffect, useLayoutEffect, useRef } from "react";
import { getCurrentWindow } from "@tauri-apps/api/window";
import { getCurrentWebview } from "@tauri-apps/api/webview";
import { api, AskTurn, listen, RoomInfo } from "../api";
...
export function useWorkspaceEffects(
  ...
  useEffect(() => {
    ...
    // KNOWN GAP (see api.electron.ts's header, "pull-progress"): this channel
    // is emitted by the Electron main process already (ollamaModels.ts /
    // moonshotCmds.ts) but is missing from events.ts's `EventPayloads`, so
    // both preload candidates' `on()` will refuse it. This call site is
    // otherwise mechanically unchanged; the fix belongs in events.ts, not
    // here. Until then this `listen` rejects (it does not throw — see the
    // adapter's `async` note), so the surrounding effect still finishes
    // registering every subscription after this one.
