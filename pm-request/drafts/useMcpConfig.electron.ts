...
// Byte-faithful port of src/settings/useMcpConfig.ts: the ONLY change is the
...
// (merged into the existing "../api" import — see that file's own doc on
// `listen` for why it is exported at all). No runtime code in this file
// changes. The one call site below (`listen<McpServerStatus[]>("mcp-status",
// ...)`) is UNCHANGED, and unlike "pull-progress" elsewhere, "mcp-status" IS
// in events.ts / both preload allowlists — nothing to flag here.
import { useEffect, useState } from "react";
import { api, ConnectorPowers, listen, McpServerStatus } from "../api";
