// ============================================================================
// ELECTRON MIGRATION DRAFT — merged. NOT WIRED IN. DO NOT MOVE INTO src/
// without separate, explicit owner authorization (electron-python-migration
// lane rules — see pm-request/electron-python-migration-plan-2026-08-22.md).
// ============================================================================
//
// A rewire of src/api.ts (the live Tauri frontend; read-only source for this
// draft) off @tauri-apps/api/core + @tauri-apps/api/event and onto the
// Electron preload bridge. Every exported symbol keeps its EXACT
// pre-migration name and signature, so this is meant to be a drop-in
// replacement for src/api.ts at cutover time.
//
// WHAT CHANGED, EXHAUSTIVELY: the two Tauri imports at the top became the
// `declare global` + `invoke`/`listen` adapter block below; two comments were
// re-worded where they named Tauri as the mechanism; a TODO was added above
// chooseOpenPath/chooseSavePath. Nothing else. All 296 `invoke<T>("cmd",
// args)` call sites and all 50 literal `listen<T>("event", cb)` call sites
// (which back this file's 59 `onXxx` subscription wrappers, 11 of them
// through the local `askEvent` helper) are UNCHANGED TEXT — only what
// `invoke`/`listen` resolve to moved.
//
// DRIFT RE-VERIFIED INDEPENDENTLY for this merge, not taken on faith: every
// one of the 295 distinct command names this file invokes is a key of
// ipc-contract.ts's `Commands` (295/295, zero misses), and every event
// channel this file listens on is a key of events.ts's `EventPayloads`
// (zero misses). See the KNOWN GAP note below for the one channel in the
// wider src/ tree that is NOT — it is not in this file.
//
// ---------------------------------------------------------------------------
// KNOWN GAP — "pull-progress" (not introduced here, NOT fixed here)
// ---------------------------------------------------------------------------
// Three call sites OUTSIDE this file (src/settings/useModelManagement.ts,
// src/workspace/effects.ts, src/viewers/ImageView.tsx — drafted alongside
// this file) subscribe to "pull-progress" directly. That channel is absent
// from events.ts's `EventPayloads` and therefore from BOTH preload
// candidates' allowlists (shared/channelAllowlist_a.ts, index_b.ts's
// KNOWN_EVENT_CHANNELS) — while the Electron main process ALREADY EMITS IT
// (electron/main/moonshotCmds.ts:338 and electron/main/ollamaModels.ts:1163,
// both `event.sender.send("pull-progress", { status, percent })`). So this is
// a one-sided contract omission, not an unbuilt feature: main sends it, the
// preload will refuse to subscribe to it, and both preload candidates' `on()`
// THROWS on an unknown channel. The fix belongs upstream in events.ts (add
// the channel; main's own payload is `{ status: string; percent: number |
// null }`), not in any of these drafts. Verified exhaustively: across all of
// src/, "pull-progress" is the ONLY listen channel missing from events.ts.
// ============================================================================

// TODO(dialog-bridge): chooseOpenPath/chooseSavePath at the bottom of this
// file still call @tauri-apps/plugin-dialog directly — there is no Electron
// dialog-surface bridge yet (separate, still-blocked design work;
// window.arcelle exposes only invoke/on, deliberately — see preload
// index_a.ts's own "SURFACE" section and index_b.ts's module doc, which both
// call dialog/shell/app explicitly out of scope for that batch). Left AS-IS
// on purpose; do not invent a bridge here. Consequence to be aware of: until
// that surface lands, this file still requires @tauri-apps/plugin-dialog at
// runtime, which an Electron build will not have — so this file cannot go
// into src/ on its own even once a cutover is authorized.
import {
  open,
  save,
...
//
// Replaces @tauri-apps/api/core's `invoke` and @tauri-apps/api/event's
// `listen`/`UnlistenFn`. Target shape taken from
// electron-migration/electron-app/electron/preload/index_a.ts AND index_b.ts
// (two independent in-progress candidates from a separate workflow) — both
// agree exactly on the surface this adapter depends on: a global
// `window.arcelle` exposing
//   invoke(channel: string, args?: unknown): Promise<unknown>
//   on(channel: string, callback: (payload: unknown) => void): () => void
// (namespace "arcelle", method names "invoke"/"on", callback receives only
// the bare payload, `on` returns a synchronous unsubscribe function).
//
// They differ in ONE way that reaches this file, and the adapters below are
// written to be correct under either: on an unknown channel, index_a's
// `invoke` returns a REJECTED PROMISE while index_b's `invoke` THROWS
// SYNCHRONOUSLY (both `on`s throw synchronously). Tauri's `invoke`/`listen`
// never threw synchronously — they always returned a promise — and all 355
// call sites below, plus the four sibling files, are written against that:
// `.catch(...)`, `await`, `unlisten.then((fn) => fn())` inside a `useEffect`
// body. A synchronous throw would escape all of those (a throw inside a
// useEffect body takes the whole component down instead of surfacing as a
// rejected promise). Marking both adapters `async` normalizes the two preload
// behaviours into the one shape every caller already handles: a synchronous
// throw from the bridge becomes a rejection. That also covers the
// preload-never-loaded case, where `window.arcelle` is undefined and the
// property read itself throws a TypeError. This is the one place the port is
// NOT a literal transcription of Tauri's implementation, and it is here
// precisely to preserve Tauri's observable contract.
//
// No `declare global` for `Window.arcelle` exists yet anywhere in
// electron-migration/ (grepped; the renderer side of this bridge is explicitly
// out of that workflow's own scope). Declared locally so this file
// type-checks in isolation; if/when a shared renderer-global d.ts exists this
// local block should be deleted in favor of it, not duplicated.
declare global {
  interface Window {
    arcelle: {
      invoke(channel: string, args?: unknown): Promise<unknown>;
...
/** Tauri's `UnlistenFn` was exactly `() => void`; restated here (not
 *  imported) now that nothing in this file depends on @tauri-apps/api/event.
 *  Kept as a named export — RecordingView.tsx and workspace/effects.ts
 *  currently import `UnlistenFn` straight from "@tauri-apps/api/event" as a
 *  type only; their own drafts (RecordingView.electron.tsx,
 *  effects.electron.ts) import it from here instead. Along with `listen`
 *  below, this is one of only TWO new exports this port adds; no
 *  pre-existing export changed name, signature or behaviour. */
export type UnlistenFn = () => void;

/** Adapter for @tauri-apps/api/core's `invoke<T>(cmd, args?)`. Every call
 *  site below keeps calling `invoke<T>("cmd", { ...args })` exactly as it did
 *  against Tauri; only this implementation changed.
 *
 *  `args` is typed `Record<string, unknown>` rather than `unknown`: that is
 *  the shape all 296 call sites below actually pass, and it keeps the
 *  argument-shape checking Tauri's own `InvokeArgs` gave them (a plain
 *  `unknown` would silently accept `invoke("cmd", "oops")`). Tauri's
 *  `InvokeArgs` also allowed `number[] | ArrayBuffer | Uint8Array`; no call
 *  site uses those today (type-checked: zero errors across the whole tree),
 *  so they are deliberately not in the union — widen it here if a binary-arg
 *  call site ever appears. Electron's IPC is structured-clone, so it would
 *  carry them fine.
 *
 *  `async` on purpose — see the block comment above on normalizing the two
 *  preload candidates' unknown-channel behaviour. */
async function invoke<T>(
  command: string,
  args?: Record<string, unknown>,
): Promise<T> {
  return (await window.arcelle.invoke(command, args)) as T;
}

/** Adapter for @tauri-apps/api/event's `listen<T>(event, cb)`. Tauri's
 *  `listen` was already async (`Promise<UnlistenFn>`) and handed the callback
 *  an envelope `{ payload }` rather than the bare value — `window.arcelle.on`
 *  is synchronous and hands the bare payload, so this wraps both: the
 *  unsubscribe function in an already-resolved Promise, and the payload back
 *  into `{ payload }` before calling `cb`. Every `listen<T>("event", (e) =>
 *  ...)` call site below is therefore unchanged.
 *
 *  Exported (the Tauri original was not — this file only ever imported it)
 *  because four other files outside api.ts (useMcpConfig.ts,
 *  useModelManagement.ts, ImageView.tsx, workspace/effects.ts) called
 *  `@tauri-apps/api/event`'s `listen` directly for one channel apiece
 *  ("mcp-status", "pull-progress" ×3) rather than going through an `api.onXxx`
 *  wrapper. Exporting this lets each of those four keep their exact call
 *  shape (`listen<T>("channel", (e) => ...)`) and change only their import's
 *  source — the same near-1:1 rewire this whole file gets, rather than
 *  rewriting them onto a different pattern (e.g. `window.arcelle.on`
 *  directly, which is synchronous and would force every one of those call
 *  sites' cleanup code to change shape too). See the KNOWN GAP note in this
 *  file's header for the "pull-progress" channel itself — a real contract gap
 *  unrelated to this adapter.
 *
 *  `async` on purpose — see the block comment above. Note the honest
 *  consequence: subscribing to a channel the preload does not know now yields
 *  a REJECTED promise rather than a thrown exception. It is deliberately not
 *  swallowed; a channel that cannot be subscribed to should be loud, and the
 *  three "pull-progress" call sites are exactly why. */
export async function listen<T>(
  event: string,
  cb: (e: { payload: T }) => void,
): Promise<UnlistenFn> {
  return window.arcelle.on(event, (payload) => cb({ payload: payload as T }));
}
...
export const api = {
  ...
  // ---- events (window.arcelle.on, via the local `listen` adapter above) ----
  ...
  // TODO(dialog-bridge): these two are LEFT AS-IS, still calling the Tauri
  // plugin — the only unmigrated calls in this file, on purpose. There is no
  // Electron dialog-surface bridge yet; that is separate, still-blocked
  // design work, not this draft's to invent. See the fuller note at the top
  // of this file, including the runtime consequence for a cutover.
 ...
 * Ported from Tauri, which mapped snake_case Rust params → camelCase invoke
 * keys; the Electron main-side registry (electron/main/ipc/registry_b.ts)
 * takes over that same camelCase-args convention — see ipc-contract.ts's
 * `Commands` interface, which api.ts's invoke<T>() calls were verified
 * against with zero drift.
 * ============================================================ */
