...
// Byte-faithful port of src/viewers/ImageView.tsx: the ONLY change is the
// import source below. `listen` used to come from @tauri-apps/api/event; it
// is now the same-named, same-signature adapter api.electron.ts exports
// (merged into the existing "../api" import). No runtime code in this file
// changes. See the KNOWN GAP comment at the one call site below.
import { useEffect, useRef, useState } from "react";
import { api, ImageBox, listen, recommendedModels } from "../api";
...
export default function ImageView({ fileId, name, mime, mediaToken, dataB64, text }: Props) {
  ...
  async function getVisionHelper() {
    ...
    // here. Note this one is `await`ed inside an `async` function, so the
    // adapter's rejection surfaces as a normal throw the caller can catch —
    // unlike the two `useEffect` sites, which get an unhandled rejection.
