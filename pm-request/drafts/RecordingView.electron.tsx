...
// Byte-faithful port of src/viewers/RecordingView.tsx: the ONLY change is
// the `UnlistenFn` TYPE import below, merged into the existing "../api"
// import (it was a bare type-only import from @tauri-apps/api/event; that
// package's own `UnlistenFn` was always exactly `() => void`, and
// api.electron.ts now exports the identical type under the same name — see
// that file's own doc comment on `UnlistenFn`). No runtime code in this file
// changes.
//
// TODO(shell-bridge): `openUrl` (@tauri-apps/plugin-opener, used below to
// open the macOS screen-recording Privacy pane) is LEFT AS-IS, unmigrated.
// It is a different Tauri plugin, outside the "@tauri-apps/api/event" /
// "@tauri-apps/api/core" scope this port covers, and neither preload
// candidate exposes a `shell`/`opener` surface — both say so explicitly
// (index_a.ts's "SURFACE" section, index_b.ts's module doc). Same reasoning
// as api.electron.ts's dialog TODO: inventing that surface here would be a
// guess. Consequence: this file still requires @tauri-apps/plugin-opener at
// runtime, which an Electron build will not have.
...
import { api, By, NoteKind, RecMeta, RecSegment, RecWord, type UnlistenFn } from "../api";
