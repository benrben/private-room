...
This is the mirror image of ``services/agent-sidecar/tests/test_mcp_bridge_wire_compat.py``:
that file is a PYTHON test that spawns a TS process
(``src/main/mcpBridgeRunner.ts``) and drives it with the real
``arcelle_sidecar.mcp_client.McpClient``. Here it is a TS test
(``apps/desktop/src/main/sidecarRun.wire.test.ts``)
...
``mcp_factory`` seams -- the exact mechanism ``services/agent-sidecar/tests/test_server.py``
already uses. ``FakeChatModel``/``FakeMCP``/``Round``/``call`` are imported
from ``services/agent-sidecar/tests/conftest.py`` UNCHANGED, not reimplemented here.
...
# This file is two levels below the service root. Both the package
# (`arcelle_sidecar`) and the test-doubles module
...
# TS test spawns this with `cwd` set to `services/agent-sidecar/`, but a human running it
