...
@pytest.mark.asyncio
async def test_antigravity_empty_success_retries_once(monkeypatch) -> None:
    model = _model("antigravity-cli")
    replies = [
        "",
        json.dumps(
            {"event": "result", "result": {"response": "Recovered answer."}}
        ),
    ]
    calls = 0
    ...
    content, tool_calls, _usage = await model.stream(
        [{"role": "user", "content": "status?"}], [], on_delta
    )
...
@pytest.mark.asyncio
async def test_antigravity_two_empty_successes_fail_clearly(monkeypatch) -> None:
    model = _model("antigravity-cli")
    calls = 0
    ...
    monkeypatch.setattr(model, "_run", fake_run)
    with pytest.raises(llm.LlmError, match="failed after 2 attempts: no output"):
        await model.stream(
            [{"role": "user", "content": "status?"}], [], lambda _d: None
        )
    assert calls == 2


@pytest.mark.asyncio
async def test_frame_handoff_without_pixels_never_starts_external_engine(monkeypatch) -> None:
    ...
    monkeypatch.setattr(model, "_run", fake_run)
    with pytest.raises(llm.LlmError, match="metadata but no valid image pixels"):
        await model.stream(
...
@pytest.mark.asyncio
async def test_privacy_stripped_frame_never_starts_external_engine(monkeypatch) -> None:
    from arcelle_sidecar import privacy
...
@pytest.mark.asyncio
async def test_historical_frame_does_not_block_a_later_text_only_turn(monkeypatch) -> None:
    """Only the latest user handoff is a pending perception obligation."""
    from arcelle_sidecar import privacy

    model = _model("claude-cli")
    # The privacy pass strips the historical pixels. That must not turn an old
    # IMAGE_HANDOFF into a failure on today's ordinary text question.
    model.privacy = privacy.policy_from_payload({"active": True, "rules": []})
    spawned = False

    async def fake_run(prompt, cancel, *args, **kwargs):
        nonlocal spawned
        spawned = True
        assert kwargs.get("images") == []
        return json.dumps({"type": "result", "result": "text answer"})
    ...
    content, calls, _usage = await model.stream(
        [
            ...
            {"role": "assistant", "content": "The old frame showed a chart."},
...
@pytest.mark.asyncio
@pytest.mark.parametrize("engine", ["claude-cli", "codex-cli"])
async def test_agent_frame_pixels_reach_every_external_image_channel(
    monkeypatch, engine: str
) -> None:
    """ARC-024: ToolResult.images must survive the agent-round adapter."""
    model = _model(engine)
    seen: dict[str, object] = {}
    if engine == "claude-cli":
        reply = json.dumps({"type": "result", "result": "visible pattern"})
    else:
        reply = "\n".join(
            [
                json.dumps(
                    {
                        "type": "item.completed",
                        "item": {"type": "agent_message", "text": "visible pattern"},
                    }
                ),
    ...
    assert content == "visible pattern"
    assert calls == []
