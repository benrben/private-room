...
async def test_studio_success_requires_and_accepts_post_commit_readback() -> None:
    """ARC-005/023: a receipt plus a successful open is verifiable success."""
    ...
    mcp = FakeMCP(
        ...
        results={
            "studio_flashcards": ToolResult(text=_studio_receipt()),
            "open_file": ToolResult(text="<html>deck</html>"),
        },
    )
    out = await drive_worker(
        make_request("make flashcards from the room", max_rounds=9),
        chat,
        mcp,
        agent_id="creator.studio",
    )

    assert [name for name, _args in mcp.calls] == ["studio_flashcards", "open_file"]
    assert out.final == "DID: saved and read back deck.html."
    assert not out.final.startswith("MISSING:")


async def test_studio_receipt_without_post_commit_readback_is_missing() -> None:
    """A model-authored success sentence cannot substitute for reading the file."""
    from conftest import FakeChatModel, FakeMCP, Round, call, drive_worker, make_request
    from arcelle_sidecar.mcp_client import ToolResult
    ...
    chat = FakeChatModel(
        [
            Round(calls=[call("studio_flashcards", name="deck.html")]),
            Round(content="DID: saved deck.html."),
            # The verifier offers a correction round; declining its open_file
            # order must replace this repeated claim deterministically.
            Round(content="DID: definitely saved deck.html."),
        ]
    )
    mcp = FakeMCP(
        tools=specs(["search_room", "open_file", "studio_flashcards"]),
        results={"studio_flashcards": ToolResult(text=_studio_receipt())},
    )
    out = await drive_worker(
        make_request("make flashcards from the room", max_rounds=9),
        chat,
    ...
    assert [name for name, _args in mcp.calls] == ["studio_flashcards"]
    assert out.final.startswith("MISSING:")
    assert "could not read the artifact back" in out.final


async def test_studio_read_before_commit_is_stale_and_does_not_verify() -> None:
    from conftest import FakeChatModel, FakeMCP, Round, call, drive_worker, make_request
    from arcelle_sidecar.mcp_client import ToolResult
    ...
    chat = FakeChatModel(
        [
            ...
            Round(calls=[call("studio_flashcards", name="deck.html")]),
            Round(content="DID: saved deck.html."),
            Round(content="DID: the old read proves it."),
        ]
    )
    mcp = FakeMCP(
        tools=specs(["search_room", "open_file", "studio_flashcards"]),
        results={
            "open_file": ToolResult(text="old contents"),
            "studio_flashcards": ToolResult(text=_studio_receipt()),
        },
    )
    out = await drive_worker(
        make_request("replace deck.html with flashcards", max_rounds=9),
        chat,
        mcp,
        agent_id="creator.studio",
    )
    ...
    assert out.final.startswith("MISSING:")
    assert [name for name, _args in mcp.calls] == ["open_file", "studio_flashcards"]


async def test_workflow_success_requires_validated_yes_receipt() -> None:
    """ARC-025: save acknowledgement is not a workflow test receipt."""
    from conftest import FakeChatModel, FakeMCP, Round, call, drive_worker, make_request
    from arcelle_sidecar.mcp_client import ToolResult
    ...
    chat = FakeChatModel(
        [
            ...
            Round(content="DID: saved and ready."),
    ...
    out = await drive_worker(
        make_request("create a daily workflow", max_rounds=9),
        chat,
        mcp,
    ...
    assert [name for name, _args in mcp.calls] == [
        "list_workflows", "save_workflow", "test_workflow"
    ]
    assert out.final == "DID: saved and validated daily."


async def test_workflow_without_validation_tool_gets_deterministic_missing() -> None:
    from conftest import FakeChatModel, FakeMCP, Round, call, drive_worker, make_request
    from arcelle_sidecar.mcp_client import ToolResult
    ...
    chat = FakeChatModel(
        [
            Round(calls=[call("save_workflow", name="daily")]),
            Round(content="DID: saved, tested, and ready."),
        ]
    )
    mcp = FakeMCP(
        tools=specs(["list_workflows", "save_workflow"]),
        results={
            "list_workflows": ToolResult(text="none"),
            "save_workflow": ToolResult(text="SAVED: daily"),
        },
    ...
    assert out.final.startswith("DID: the workflow may have been saved as a draft.")
    assert "MISSING:" in out.final
    assert "VALIDATED: yes" in out.final


async def test_workflow_rejects_validated_receipt_from_before_latest_save() -> None:
    """A stale green run cannot prove a subsequently changed definition."""
    from conftest import FakeChatModel, FakeMCP, Round, call, drive_worker, make_request
    from arcelle_sidecar.mcp_client import ToolResult
    ...
    chat = FakeChatModel(
        [
            Round(calls=[call("test_workflow", name_or_id="daily")]),
            Round(calls=[call("save_workflow", name="daily")]),
            Round(content="DID: the changed workflow is tested and ready."),
            Round(content="DID: still ready."),
            Round(content="DID: definitely ready."),
        ]
    )
    mcp = FakeMCP(
        tools=specs(["list_workflows", "save_workflow", "test_workflow"]),
        results={
            "list_workflows": ToolResult(text="daily — draft"),
            "test_workflow": ToolResult(text="VALIDATED: yes"),
            "save_workflow": ToolResult(text="SAVED: daily"),
        },
    )
    out = await drive_worker(
        make_request("change the daily workflow", max_rounds=9),
        chat,
        mcp,
        agent_id="jobs.workflows",
    )
...
async def test_workflow_rejects_validated_receipt_for_different_name() -> None:
    """A green run for weekly cannot validate the newly saved daily workflow."""
    from conftest import FakeChatModel, FakeMCP, Round, call, drive_worker, make_request
    from arcelle_sidecar.mcp_client import ToolResult
    ...
    chat = FakeChatModel(
        [
            ...
            Round(content="DID: daily is tested and ready."),
            Round(content="DID: daily is definitely ready."),
        ]
    )
    mcp = FakeMCP(
        tools=specs(["list_workflows", "save_workflow", "test_workflow"]),
    ...
    out = await drive_worker(
        make_request("create the daily workflow", max_rounds=9),
        chat,
        mcp,
        agent_id="jobs.workflows",
    )
...
def test_nothing_in_the_package_sets_the_studio_deps_hook() -> None:
    ...
    pkg = Path(__file__).parent.parent / "src" / "arcelle_sidecar"
    offenders = []
