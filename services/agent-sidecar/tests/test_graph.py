...
async def test_frame_receipt_without_pixels_fails_closed() -> None:
    """ARC-024: timestamp/hash metadata is not visual model input."""
    chat = FakeChatModel(
        [
            Round(content="checking", calls=[call("view_media_frame", name="demo.mp4", timestamp="00:10")]),
            Round(content="I could not inspect the frame."),
        ]
    )
    mcp = FakeMCP(
        ...
        results={
            "view_media_frame": ToolResult(
                text="Frame receipt: 00:10, 1280x720, SHA-256 deadbeef"
            )
        },
    )
    out = await drive_worker(
        make_request("what is on screen at 00:10?", max_rounds=9),
        chat,
        mcp,
    ...
    tool_results = [m for m in out.messages if m.get("role") == "tool"]
    assert len(tool_results) == 1
    assert "no image pixels" in str(tool_results[0].get("content"))
    assert "No visual interpretation was performed" in str(
