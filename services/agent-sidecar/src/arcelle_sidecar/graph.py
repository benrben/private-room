...
async def prepare(state: AgentState, config: RunnableConfig) -> dict[str, Any]:
    ...
    if messages and messages[0].get("role") == "system" and not no_tools:
        ...
        if (
            ...
            and (
                agent.main
                or bool(set(agent.tools) & offered)
                # files.read is deliberately reachable from CORE alone. On a
                # read-only/provider tier none of its optional organize verbs
                # are offered, but that must not silently remove FILES_PROMPT's
                # grounding/attribution rules (ARC-006/007/030).
                or (agent.core_capable and bool(set(CORE_TOOLS) & offered))
            )
...
class _ToolPass:
    ...
    async def _room_tool(self, call: ToolCall, key: str) -> bool:
        ...
        # Treating its timestamp/hash text as a successful perception result lets
        # a text-only model infer pixels it never received.  Electron normally
        # returns both parts, but this is the last trust boundary before the model:
        # a bridge regression or a privacy transform that drops ``images`` must
            ...
            if call.name in LEDGER_TOOLS:
                for artifact in _referent_names(call.name, call.arguments, result):
                    ...
                    if call.name in {
                        ...
                        # A successful commit invalidates an earlier read of the
                        # same name. Allow open_file to run again so the verifier
                        # can obtain a genuinely post-commit receipt instead of
                        # duplicate-suppressing the only proof it accepts.
                        self.seen.discard(
                            ToolCall(
                                name="open_file", arguments={"name": artifact}
                            ).key()
                        )
        else:
...
async def run_agent(req: RunRequest, deps: Deps) -> str:
    ...
    if tagged and tool_policy != "none":
...
async def stream_events(req: RunRequest, deps_factory: Callable[[Emit], Deps]):
    ...
    async def driver() -> None:
        try:
            if req.harness == "deep" and req.resolved_tool_policy() != "none":
