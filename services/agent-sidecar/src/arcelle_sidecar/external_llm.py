...
from .prompts import IMAGE_HANDOFF, READ_RESULT_TOOL
...
class ExternalChatModel:
    ...
    async def stream(
        ...
        frame_handoff = bool(
            ...
            and str(latest_user.get("content") or "").strip() == IMAGE_HANDOFF
        )
        original_images = message_images(latest_user) if latest_user is not None else []
        if frame_handoff and not original_images:
            raise LlmError(
                "ENGINE_ERROR",
                "The perception tool returned metadata but no valid image pixels, "
                "so visual interpretation stopped before model dispatch.",
            )
        if frame_handoff and self.engine not in IMAGE_ENGINES:
            raise LlmError(
                "ENGINE_ERROR",
                f"{self.engine} has no image input channel; the captured image "
                "was not dispatched.",
            )
        ...
        latest_send_user = next(
            (msg for msg in reversed(send) if msg.get("role") == "user"),
            None,
        )
        delivered_handoff_images = (
            message_images(latest_send_user)
            if latest_send_user is not None
            and str(latest_send_user.get("content") or "").strip() == IMAGE_HANDOFF
            else []
        )
        if frame_handoff and not delivered_handoff_images:
            raise LlmError(
                "ENGINE_ERROR",
        ...
        try:
            ...
            stdout = await invoke()
            if (
                self.engine == "antigravity-cli"
                and not parse_antigravity_json_stream(stdout)[0].strip()
                and not _stopped(cancel)
            ):
                # ARC-029: a successful exit with an empty envelope is a
                # transient adapter failure, not an answer.  Retry once only;
                # no Arcelle tool has executed yet because Antigravity's tools
                # are parsed from that envelope after the process returns.
                stdout = await invoke()
                if not parse_antigravity_json_stream(stdout)[0].strip():
                    raise LlmError(
                        "ENGINE_ERROR",
                        "antigravity-cli failed after 2 attempts: no output "
                        "(exit status 0).",
                    )
