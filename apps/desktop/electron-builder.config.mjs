...
const sidecarStageDir = resolveResourceDir(
  ...
  "the built Python agent sidecar (PyInstaller onedir; services/agent-sidecar/build-sidecar.sh's real output, out of this batch's scope)",
);

// Real, committed source-of-record assets -- READ-ONLY references (never
// copied, never edited) in this Electron package's resources/icons directory.
...
const documentIcns = path.join(here, "resources", "icons", "document.icns");
const appIcon = path.join(here, "resources", "icons", "icon.icns");
...
const extendInfo = {
  ...
  // capture (src/renderer/platform/recording/loopbackTap.ts) rides the Screen & System
