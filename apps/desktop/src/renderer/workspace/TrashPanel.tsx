...
function TrashRow({
  ...
  return (
    <div className={`trash-row${picked ? " is-picked" : ""}`}>
      <input
        type="checkbox"
        className="trash-row-check"
        checked={picked}
        aria-label={`Select ${displayName(file.name)}`}
        onChange={() => toggle(file.id)}
      />
      <div className="trash-row-main">
        <span className="trash-row-name" dir="auto" title={file.name}>
          {displayName(file.name)}
        </span>
        <span className="trash-row-meta">
          {whenLabel(file.trashedAt)} · {actorLabel(file.trashedBy, file.trashedById)}{" "}
      ...
      <div className="trash-row-actions">
        <button
          className="chip-btn"
          title={`Put "${displayName(file.name)}" back in the library`}
          aria-label={`Restore ${displayName(file.name)}`}
          onClick={() => void a.restoreFile(file.id)}
        >
          <UndoIcon size={14} />
        </button>
        ...
        <DeleteControl
          k={`trash-destroy-${file.id}`}
          trigger={<TrashIcon size={14} />}
          question="Delete for good? This cannot be undone."
          title={`Delete "${displayName(file.name)}" for good`}
          onConfirm={() => void a.destroyFile(file.id)}
          confirmDelete={s.confirmDelete}
          askConfirm={a.askConfirm}
          cancelConfirm={a.cancelConfirm}
        />
...
function TrashFooter({ s, a }: { s: WSState; a: WSActions }) {
  if (s.confirmDelete !== "trash-empty-all") {
    return (
      <button
...
export default function TrashPanel({
  ...
  return (
    <div className="library-scroll trash-list">
      ...
      <TrashSelectionBar picked={picked} s={s} a={a} />
      {shown.map((file) => (
        <TrashRow
          key={file.id}
          file={file}
          picked={s.selectedTrashIds.has(file.id)}
          s={s}
          a={a}
          toggle={toggle}
        />
