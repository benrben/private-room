...
describe("gatherContextAndSaveQuestion", () => {
  ...
  it("starts directly tagged specialists without unrelated ambient room sources", () => {
    const db = freshRoom();
    insertFile(db, "notes.md", "text/markdown", Buffer.from("scripts and jobs notes"), "scripts and jobs notes", "upload");
    insertFile(db, "image.png", "image/png", Buffer.from("not pixels"), "unrelated image", "upload");
    const ctx = gatherContextAndSaveQuestion(
      db,
      randomUUID(),
      "*scripts list the scripts you can run; do not read room files",
      [],
      [0.1, 0.2],
      null,
    );
    expect(ctx.sources).toEqual([]);
    expect(ctx.chatMessages.at(-1)?.content).not.toContain("[file:");
    expect(ctx.chatMessages.at(-1)?.content).not.toContain("Recently added content");
  });

  it("still grounds a tagged specialist in explicitly named files", () => {
    const db = freshRoom();
    insertFile(db, "safe-test-script.py", "text/x-python", Buffer.from("print('safe')"), "print('safe')", "upload");
    const ctx = gatherContextAndSaveQuestion(
      db,
      randomUUID(),
      "*scripts inspect safe-test-script.py",
      [],
      null,
      null,
    );
    expect(ctx.sources).toEqual(["safe-test-script.py"]);
    expect(ctx.chatMessages.at(-1)?.content).toContain("[file: safe-test-script.py]");
  });
