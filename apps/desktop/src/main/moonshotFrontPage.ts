 ...
 *    endpoint is implemented (`services/agent-sidecar/src/arcelle_sidecar/server.py`,
...
import { SECTION_ONLY, listPublicFiles, type FileMeta as DbFileMeta } from "./db-host/files.js";
...
export function frontPageOf(db: Database.Database): FrontPage {
  // This is the same renderer-visible population `list_files` returns to the
  // footer and Library surfaces. Hidden derived previews are implementation
  // bytes, not extra documents, so neither the Home count nor its recent strip
  // may reveal them as additional files.
  const publicFiles = listPublicFiles(db);
  const recentFiles = publicFiles
  ...
  const fileCount = publicFiles.length;
  // Both counts propagate rather than falling back to 0 — a query failure
  // here throws (via `queryOne`), matching `listPublicFiles`'s own throw-on-error
