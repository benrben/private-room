...
export async function probeOpenrouterModelSelection(
  ...
  if (exactId === "") return { ok: false, detail: "Choose a specific OpenRouter model first." };
  let key: string;
  try {
    key = deps.readKey(OPENROUTER_ID);
  } catch {
    return { ok: false, detail: "No OpenRouter API key is saved on this Mac." };
  }
  try {
    const response = await deps.fetchJson(`${OPENROUTER_BASE_URL}/chat/completions`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${key}`,
    ...
    if (response.status === 401) noteKeyRejected(OPENROUTER_ID);
    const message =
      jstr(jget(jget(value, "error"), "message"))
      ?? jstr(jget(value, "error"))
      ?? "the provider rejected this model";
    return { ok: false, detail: `OpenRouter HTTP ${response.status}: ${message}` };
  } catch (error) {
    return {
      ok: false,
