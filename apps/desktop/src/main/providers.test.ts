...
describe("probeOpenrouterModelSelection", () => {
  ...
  it("posts the exact provider ID with a one-token budget", async () => {
    ...
    const result = await probeOpenrouterModelSelection(" vendor/exact-id ", forbiddenDeps({
      readKey: () => "test-key",
      fetchJson: async (url, init) => {
        seenUrl = url;
        seenInit = init;
        return jsonResponse(200, { choices: [{ message: { content: "O" } }] });
      },
    }));
    expect(result).toEqual({ ok: true, detail: null });
    expect(seenUrl).toBe(`${OPENROUTER_BASE_URL}/chat/completions`);
    expect(seenInit).not.toBeNull();
    const init = seenInit as unknown as RequestInit;
    expect(init.method).toBe("POST");
    expect(JSON.parse(init.body ?? "{}")).toMatchObject({
      model: "vendor/exact-id",
      max_tokens: 1,
    });
  });

  it("turns a provider runtime rejection into a disabled-selection reason", async () => {
    await expect(probeOpenrouterModelSelection("openai/bad", forbiddenDeps({
      readKey: () => "test-key",
      fetchJson: async () => jsonResponse(400, { error: { message: "invalid model name" } }),
    }))).resolves.toEqual({
      ok: false,
