...
describe("RoomToolDispatcher.listTools", () => {
  ...
  it("fails closed when batch file tools could bypass a Cloud Privacy mirror", async () => {
    ...
    expect(advertised).not.toContain("save_workflow");
    expect(advertised).not.toContain("update_workflow");
    expect(advertised).not.toContain("delete_workflow");
    expect(advertised).not.toContain("retranscribe_file");
    expect(advertised).not.toContain("view_media_frame");
    expect(advertised).not.toContain("view_screenshot");
    expect(advertised).toContain("ui_snapshot");
    ...
    for (const [name, args] of [
      ...
      ["save_workflow", { name: "Daily notes", definition: { version: 1, nodes: [], edges: [] } }],
      ["update_workflow", { name: "Daily notes", definition: { version: 1, nodes: [], edges: [] } }],
      ["delete_workflow", { name: "Daily notes" }],
      ["retranscribe_file", { name: "audio.m4a" }],
      ["view_media_frame", { name: "video.mp4", at: "0:01" }],
      ["view_screenshot", {}],
    ] as const) {
