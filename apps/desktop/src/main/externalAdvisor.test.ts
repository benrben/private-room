...
describe("the production shell and working directory", () => {
  ...
  it("retries Antigravity once when it exits successfully without a terminal answer", async () => {
    let calls = 0;
    const spawnFn: AdvisorSpawnFn = () => {
      ...
      proc.stdin.on("end", () => {
        calls += 1;
        proc.stdout.end(calls === 1
          ? `${JSON.stringify({ event: "init", conversation_id: "empty-first" })}\n`
          : `${JSON.stringify({ event: "result", result: { status: "SUCCESS", response: "AAPL is 314.58 USD" } })}\n`);
        proc.stderr.end();
        proc.emit("close", 0, null);
      });
      return proc;
    };
    await expect(runExternalCli("antigravity-cli", [userMsg("read AAPL")], { spawnFn }))
      .resolves.toMatchObject({ text: "AAPL is 314.58 USD" });
    expect(calls).toBe(2);
  });

  it("surfaces exit/stdout/stderr diagnostics when both Antigravity attempts are empty", async () => {
    const spawnFn: AdvisorSpawnFn = () => {
      const proc = new FakeProcess();
      proc.stdin.resume();
      proc.stdin.on("end", () => {
        proc.stdout.end(`${JSON.stringify({ event: "init", conversation_id: "still-empty" })}\n`);
        proc.stderr.end("provider emitted no terminal result");
        proc.emit("close", 0, null);
      });
      return proc;
    };
