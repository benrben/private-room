...
describe("dialogOpen", () => {
  ...
  it("allows a room picker to select either a workspace folder or a legacy room file", async () => {
    const deps = fakeDeps();
    await dialogOpen(deps, {
      room: true,
      message: "Select the room folder, then click Open Room.",
      buttonLabel: "Open Room",
    });
    expect(optionsOf(deps.showOpenDialog)).toMatchObject({
