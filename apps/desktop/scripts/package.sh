...
# `src/main/index.electron.test.ts`'s own "NATIVE-MODULE BOUNDARY" doc
...
# a signing identity when this var is unset). Local proof builds opt out of
# that search so a certificate is never selected accidentally. A release run
# with an explicit intent to sign enables discovery itself:
# CSC_IDENTITY_AUTO_DISCOVERY=true scripts/package.sh
