...
async function writeFixtureTree(root, { electronAppVersion, rootPkgVersion, rootLockVersion, sidecarVersions }) {
  const electronApp = path.join(root, "apps", "desktop");
  await mkdir(electronApp, { recursive: true });
  await writeFile(path.join(electronApp, "package.json"), JSON.stringify({ name: "@arcelle/desktop", version: electronAppVersion }));
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      packages: {
        "": { version: rootLockVersion },
        "apps/desktop": { version: electronAppVersion },
      },
    }),
  );
  const sidecar = path.join(root, "services", "agent-sidecar");
  await mkdir(path.join(sidecar, "src", "arcelle_sidecar"), { recursive: true });
  await writeFile(
    path.join(sidecar, "pyproject.toml"),
    `[project]\nname = "arcelle-sidecar"\nversion = "${sidecarVersions.pyproject}"\n`,
  );
  await writeFile(
    path.join(sidecar, "src", "arcelle_sidecar", "__init__.py"),
...
describe("checkVersionFiles — fixture trees", () => {
  it("reports ok:true when every workspace version agrees with the desktop package", async () => {
    ...
    // 1 source + 1 root package + 3 workspace-lock keys + 3 sidecar files = 8.
  ...
  it("treats a missing file as a failed check (found: null), not a thrown error", async () => {
    ...
    await rm(path.join(root, "services", "agent-sidecar", "uv.lock"));
    const result = await checkVersionFiles({ repoRoot: root });
    expect(result.ok).toBe(false);
...
describe("checkVersionFiles — live repo", () => {
  ...
  it("keeps every Electron-era version file in agreement", async () => {
    ...
    expect(result.version).toBe("0.26.11");
    expect(result.ok).toBe(true);
    expect(result.checks.every((check) => check.found === "0.26.11")).toBe(true);
  });
});
