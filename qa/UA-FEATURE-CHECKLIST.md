...
**How to use each item:** exercise the control via the stated path, observe the stated outcome, and only then check it off. If a precondition can't be met in the test environment, mark the item *blocked-precondition* rather than skipping silently. File references (`file.tsx`) are for debugging failures, not part of the test — paths are relative to `src/` (or `src-tauri/src/` for `.rs`). **They name a FILE, and a SYMBOL where one helps** (`` `agent.rs` `claims_unbacked_action` ``) — never a line number. On 2026-08-03 all 387 line ranges in this sheet were stripped: every one sampled in two separate audits landed on unrelated code, because line numbers drift with every wave of edits and a stale range sends a tester chasing a failure into the wrong place. Do not add them back.
...
- [ ] `*` specialist menu: typing `*` lists the agents THIS room can dispatch to (File/Web/App/Jobs/Skills/Connector — a web-off room shows no Web agent); filter by key, label or area; ↑↓/Enter/Tab/Esc and mouse insert; "* Specialist" chip opens it (`ComposerPane.tsx`, `composer.ts` `specialistItems`).
...
- [ ] Sending `*web …` runs ONLY that specialist — the agent diagram shows the Web agent and no other; `*web` in a web-off room is refused by name in the answer rather than answered by a different agent (`agents.agent_tool_specs(only=…)`).
...
- [ ] …and the sidecar holds the SAME policy, so a tag that gets past the composer (roster still loading, headless `agent_run`) is refused BY NAME in the answer, with the valid tags listed — never quietly run as an untagged turn (`agents.tagged_specialist`, `prompts.TAG_UNAVAILABLE_NOTE`).
...
- [ ] Nothing here is hand-maintained: adding an agent in `sidecar/arcelle_sidecar/agents.py` makes a new row appear with no frontend change.
...
- [ ] **Read as text** (toolbar, or the skip button) opens a reading panel that SHRINKS the page beside it — never covers it. Its heading takes focus; the text has real headings, real lists and real tables. Links inside it navigate the private browser, never the system browser.
