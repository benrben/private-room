...
const FONTS = read("apps/desktop/src/renderer/styles/fonts.css");
const TOKENS = read("apps/desktop/src/renderer/styles/tokens.css");
const FONT_DIR = "apps/desktop/src/renderer/assets/fonts";

/** Every stylesheet the app ships, as [path, text]. */
function sheets() {
  const out = [];
  for (const dir of ["apps/desktop/src/renderer/styles", "apps/desktop/src/renderer/viewers"]) {
...
test("nothing asks for Manrope any more", () => {
  ...
  for (const [path, css] of [
    ...sheets(),
    ["apps/desktop/index.html", read("apps/desktop/index.html")],
  ]) {
...
test("the hand is spent on words a person wrote, not on computed facts", () => {
  ...
  uses += (read("apps/desktop/src/renderer/icons/nav.tsx").match(/var\(--hand\)/g) || []).length;
  assert.equal(
    uses,
    27,
...
test("…and the same census over the classes, not just the token", () => {
  ...
  walk("apps/desktop/src/renderer");
  assert.deepEqual(
    found,
    WEARERS,
