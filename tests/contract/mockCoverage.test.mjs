 ...
 *
 * `tests/support/check-mock-coverage.mjs` is a build gate (preflight runs it), and its
...
const CHECKER = path.join(here, "../../tests/support/check-mock-coverage.mjs");
...
function writeTree(dir, screenBody) {
  ...
  w(
    "tests/support/qa-mock.js",
    `const commands = {\n    list_things: () => [],\n};\nconst EXTRA_READS = new Set(["list_things"]);\n`,
  );
  w("apps/desktop/src/renderer/api.ts", API_TS);
  w("apps/desktop/src/renderer/Screen.tsx", screenBody);
}
...
test("no orphan, no failure", () => {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(dir, "apps/desktop/src/renderer/api.ts"),
      API_TS.replace(/\n +forgottenThing:[^\n]*\n/, "\n"),
    );
    const r = spawnSync(process.execPath, [CHECKER], {
      encoding: "utf8",
