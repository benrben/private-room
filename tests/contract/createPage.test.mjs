...
const {
  ...
  await load("apps/desktop/src/renderer/workspace/create/selectors.ts");
...
test("the create job kind is dispatchable and resumable, not just startable", () => {
  ...
  assert.match(
    read("apps/desktop/src/main/jobQueue.ts"),
    /"create"/,
...
test("the running-job card states the job's label and no figure of its own", () => {
  ...
  // for.
  const page = read("apps/desktop/src/renderer/workspace/create/CreatePage.tsx");
  assert.doesNotMatch(
    page,
