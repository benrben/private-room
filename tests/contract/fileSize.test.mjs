...
test("the screens that rolled their own now use it", () => {
  ...
  const ck = readFileSync(join(root, "apps/desktop/src/renderer/settings/CheckpointsSection.tsx"), "utf8");
  assert.ok(!/function formatBytes/.test(ck), "CheckpointsSection kept a private formatter");
  assert.match(ck, /formatSize\(/);
  const skills = readFileSync(join(root, "apps/desktop/src/renderer/workspace/skills/SkillsView.tsx"), "utf8");
  assert.ok(
...
test("the cloud preview sizes the payload in UTF-8 bytes, not characters", () => {
  ...
  const cloud = readFileSync(join(root, "apps/desktop/src/renderer/viewers/CloudView.tsx"), "utf8");
  assert.ok(
