...
const DESTINATIONS = read("apps/desktop/src/renderer/workspace/destinations.ts");
...
test("⌘T and ⌘W have ONE owner, and it is the native menu", () => {
  ...
  const NATIVE = read("apps/desktop/src/renderer/shell/useNativeMenu.ts");
...
test("a page with no address yet is named the way Rust names it", () => {
  ...
  assert.match(
    read("apps/desktop/src/main/browser/tabs.ts"),
    /return "New page"/,
  );
});
...
const toasts = await load(read("apps/desktop/src/renderer/workspace/toastStack.ts"));
