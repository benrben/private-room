...
test("closing Settings with unsaved work asks first", () => {
  ...
  assert.match(
    read("apps/desktop/src/renderer/settings/useBehaviorSettings.ts"),
    /tuningDirty:/,
    "the instructions/creativity fields report no dirty state",
  );
  assert.match(read("apps/desktop/src/renderer/settings/useVoiceSettings.ts"), /voiceDirty:/);
  assert.match(read("apps/desktop/src/renderer/settings/useRemoteAi.ts"), /closetDirty:/);
