...
const { parseEml, decodeQuotedPrintable, decodeMimeWords } = await load("apps/desktop/src/renderer/viewers/eml.ts");
const { parseCues, parseStamp, formatStamp, toSrt, toVtt } = await load(
  "apps/desktop/src/renderer/viewers/subtitles.ts",
);
const { resolvePath, withoutFragment, mimeForPath, isRenderableImage } = await load(
  "apps/desktop/src/renderer/viewers/zipdoc.ts",
);
