/* Visual-QA app-bridge mock. Loaded BEFORE the app bundle in
 * apps/desktop/dist/qa.html so the real UI renders with realistic data in an
 * ordinary browser (no Electron main process). Never shipped: qa.html is generated on demand by tests/support/make-qa.mjs
...
(() => {
   ...
   *   node tests/support/check-mock-coverage.mjs   lists the gap without running the app. */
  ...
  const commands = {
    ...
    engine_support_matrix: () => ({
      ...
      agents: [
        ...
        { id: "app.design", label: "Design agent" },
      ],
      providers: [
        {
          ...
          available: true, agents: ["files.read", "chat.web", "app.ui", "app.design"],
        },
        {
        ...
        },
        {
          ...
          tier: "cloud-engine", imageReaches: false,
          available: false, agents: ["files.read", "chat.web", "app.ui", "app.design"],
        },
      ],
    }),
   ...
   * (apps/desktop/src/preload/index_a.ts and
   ...
   * docs). `src/shared/ipc-contract.ts`'s `Commands` interface is the
