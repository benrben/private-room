...
const OUT = path.resolve(__dirname, "..", "..", ".artifacts", "test-results", "capture", "vision");
 ...
 * does.
 *
 * `?qa_state=` only reaches commands `tests/support/qa-mock.js` recognises as reads. It
    ...
    if (unreached.size) {
      ...
      console.log(
        `  why: either that pane has no data of its own (a static start screen ` +
          ...
          `tests/support/qa-mock.js counts as a read — see the EXTRA_READS set there, ` +
