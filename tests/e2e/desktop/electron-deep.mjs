...
import { runArcLiveRegression } from "../../support/arcLiveRegression.mjs";

const HERE = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(HERE, "..", "..", "..");
const ELECTRON_APP = path.join(ROOT, "apps", "desktop");
const SIDECAR = path.join(ROOT, "services", "agent-sidecar");
const SIDECAR_PYTHON = path.join(SIDECAR, ".venv", "bin", "python");
const ENTRY = path.join(ELECTRON_APP, "dist_package", "src", "main", "index.js");
...
try {
  ...
  const reachableSpecialists = specialists.filter(({ capability }) => capability !== "unavailable");
  ...
  for (const specialist of reachableSpecialists) {
  ...
  assert.equal(routedKeys.size, new Set(reachableSpecialists.map(({ key }) => key)).size);
  log(`agents: ${specialists.length} canonical rows discovered; every reachable *specialist route passed`);
