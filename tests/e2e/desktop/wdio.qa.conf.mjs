// WebdriverIO config for the REGRESSION suite (tests/e2e/qa/*.e2e.mjs).
//
// This config drives the production React app in Chrome against dist/qa.html,
// where tests/support/qa-mock.js stands in for the Electron IPC backend.
//
//   npm run build  ──▶ dist/index.html
//   tests/support/make-qa.mjs ──▶ dist/qa.html  (= index.html + the IPC mock, injected first)
...
export const config = {
  runner: "local",
  specs: [path.join(projectRoot, "tests", "e2e", "qa", "*.e2e.mjs")],
  maxInstances: 1,
  ...
  onPrepare: async () => {
    ...
    try {
      if (!process.env.SKIP_BUILD) {
        ...
        run(process.execPath, [path.join(projectRoot, "tests", "support", "make-qa.mjs")], "make-qa");
      }
