...
export default async function afterSign(context) {
  ...
  const osxSignEntitlements = path.resolve(here, "..", "node_modules", "@electron", "osx-sign", "entitlements");
  const helperSpecs = [
  ...
  // sidecar/sidecar-entitlements.plist is real, committed, load-bearing
  // source -- read-only reference, never copied or edited by this candidate.
  const sidecarEntitlementsPath = path.resolve(here, "..", "..", "..", "sidecar", "sidecar-entitlements.plist");
