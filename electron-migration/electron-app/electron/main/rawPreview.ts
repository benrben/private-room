...
/** Return the largest structurally-valid embedded JPEG, or `null`. */
export function extractRawPreview(bytes: Uint8Array): RawJpegPreview | null {
  ...
  for (const [start, parsed] of found) {
    if (best === undefined || parsed.end - start > best.parsed.end - best.start) best = { start, parsed };
  }
