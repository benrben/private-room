    ...
    try {
      ...
      switch (name) {
        ...
        case "stt_status": {
          ...
          const lane = sttBusy.size ? ` Transcribing ${[...sttBusy].join(", ")} right now.` : " Nothing is transcribing right now.";
        ...
        case "retranscribe_file": {
          const [fileId, realName] = findFileLike(room.conn, str(args.name));
          sttBusy.add(realName);
          void retranscribeFile(state, userDataDir, resourcesPath, emit, fileId)
            .catch((error) => emit("stt-progress", {
              fileId,
              label: error instanceof Error ? error.message : String(error),
              failed: true,
            }))
            .finally(() => sttBusy.delete(realName));
          return ok(`Queued ${realName} for on-device re-transcription. Call stt_status to see whether it is still busy.`);
        }
        case "read_recording": {
          if (!roomDeps.jobQueue) throw new Error("The background job queue is unavailable.");
          const [fileId, realName] = findFileLike(room.conn, str(args.name));
