 ...
 * PNG and never call Quick Look again. Concurrent calls share one render. */
export function snapshotUnknownFormat(
  ...
  const task = (async (): Promise<DerivedPreviewStoreResult> => {
    ...
    let png: Buffer | null;
    try {
      png = await render(original.name, original.bytes);
    } catch {
    ...
    if (png === null) return { kind: "unavailable" };
    return storeDerivedPreview(room, originalId, png, "image/png", "png");
