 ...
 * streaming client.
 *
 * The mirror image of `sidecar/tests/test_mcp_bridge_wire_compat.py`: that
 ...
 * `sidecar/tests/test_server.py` already uses (via
 * `sidecar/tests/tools/run_wire_compat_server.py`; read that file's own doc
...
// electron/main/ -> the repo root is 4 levels up, the same walk
// `defaultDevSidecarDir` documents (and which collapses to a single
// `../sidecar` once this file moves to its post-cutover location).
const HERE = path.dirname(fileURLToPath(import.meta.url));
const SIDECAR_DIR = path.resolve(HERE, "../../../../sidecar");
