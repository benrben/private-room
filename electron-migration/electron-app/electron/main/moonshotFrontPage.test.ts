...
describe("frontPageOf", () => {
  it("the front page count is the same count everyone else shows", () => {
    const db = freshRoom();
    ...
    // `listFiles` is what this page's own strip is drawn from — NOT the
    // Library badge, which is a narrower question.
    expect(fp.fileCount).toBe(listFiles(db).length);
